import uvicorn
from fastapi import APIRouter, Request, Header, HTTPException, status, Request, FastAPI
...
routersDir = (os.path.dirname(scriptDir)+"/api_validations")
...
async def login_user(login: Login):
    ...
    loop = asyncio.get_running_loop()
    token = await loop.run_in_executor(ThreadPoolExecutor(), save_user_token, user_id)
    return Response(
...
async def register_user(register: Register):
    ...
    try:
        ...
        city_code = await find_country_code(country_code, False, register.city_name)
